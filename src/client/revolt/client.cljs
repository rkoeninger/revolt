...
(defn adjust-bid [data id denomination adj]
  (let [bid-denom-adjusted (+ (get-in @data [:bids id denomination]) adj)
    ...
    (when (and (>= bank-denom-adjusted 0) (>= bid-denom-adjusted 0))
      (swap-in! data [:remaining-bank denomination] #(- % adj))
      (swap-in! data [:bids id denomination] #(+ % adj)))))
...
(defn denom [data bank denomination]
  (let [amount (get bank denomination)]
    (if (and amount (pos? amount))
      (spjoin amount (localize data denomination)))))
...
(defn guard-house-row [data]
  (let [guard-house (:guard-house @data)]
    (if (= :palace (:expansion @data))
      (h/tr
        (h/th {:colSpan 2} (localize data :guard-house))
        (each (:players @data)
          #(if (= guard-house (:id %)) check-mark))))))

(defn influence-row-template [data f {:keys [id cap]}]
  (h/tr
    (h/th {:class "location-name"} (localize data id))
            ...
            (fn [lid pid]
              (let [amount (get-in @data [:influence lid pid])
                ...
                (if (and (pos? amount) (not= pid guard-house-occupant))
                  (h/button amount
                    {:onClick #(reset-in! data [:spy-selection] [lid pid])
  ...
  (let [selection (:spy-selection @data)]
    [(command-button
      ...
      #(when selection
        ...
        (reset-in! data [:spy-selection] nil)))
     (command-button
      data
      "spy-clear-button"
      (localize data :clear)
                  ...
                  (h/button amount
                    {:class (if selected "selected")
                     :onClick #(if selection-1
                                  (reset-in! data [:apothecary-selection-2] [lid pid])
                                  (reset-in! data [:apothecary-selection-1] [lid pid]))}))))))))))
  ...
  (let [selection-1 (:apothecary-selection-1 @data)
    ...
    [(command-button
      ...
      #(when (and selection-1 selection-2)
        ...
        (reset-in! data [:apothecary-selection-1] nil)
        (reset-in! data [:apothecary-selection-2] nil)))
     (command-button
      data
              ...
              (let [selected (or (= lid selection-1) (= lid selection-2))
                ...
                (h/tr
                  (h/th {:class "location-name"}
                    (cond
                      selected (h/span {:class "selected"} (localize data lid))

                      ; max reassignments made
                      (>= (count reassignments) 2) (localize data lid)
                      ...
                      (if (zero? total-influence)
                        (localize data lid)
                        (h/button (localize data lid)
                          {:onClick #(do (reset-in! data [:messenger-selection-1] lid))}))
                      ...
                      (if (= cap total-influence)
                        (localize data lid)
                        (h/button (localize data lid)
                          {:onClick #(do (reset-in! data [:messenger-selection-2] lid))}))

                      ; pending add
                      :else (localize data lid)))
                  (h/td {:class "influence-cap"} cap)
                  (each (:players @data)
    ...
    [(if (pos? (count reassignments))
      (h/ul
        (each reassignments
          (fn [[lid1 lid2]]
            (h/li
              [(h/span {:class "location-name"} (localize data lid1))
               " → "
               (h/span {:class "location-name"} (localize data lid2))])))))
     (command-button
      data
      ...
      #(do
        ...
        (reset-in! data [:messenger-reassignments] nil)
        (reset-in! data [:messenger-selection-1] nil)
        (reset-in! data [:messenger-selection-2] nil)))
     (command-button
      data
      ...
      (not (and selection-1 selection-2))
      #(do
        (reset-in! data [:messenger-reassignments] (conj reassignments [selection-1 selection-2]))
        (reset-in! data [:messenger-selection-1] nil)
        (reset-in! data [:messenger-selection-2] nil)))
     (command-button
      data
                ...
                (h/tr
                  (h/th {:class "location-name"}
                    (if capped
                      ...
                      (h/button
                        {:class (if selected "selected")
                         :onClick #(reset-in! data [:mayor-selection] lid)}
                        (localize data lid))))
                  (h/td {:class "influence-cap"} (localize data cap))
                  (each (:players @data)
                    (fn [{pid :id}] (h/td (dont-show-zero (get-in @data [:influence lid pid]))))))))))))))
  ...
  (let [selection (:mayor-selection @data)]
    [(command-button
      ...
      #(when selection
        ...
        (reset-in! data [:mayor-selection] nil)))
     (command-button
      data
      "mayor-clear-button"
      (localize data :clear)
...
(defn player-list [data]
  (h/ol {:class "player-list"}
    (map (comp h/li :name) {:players @data})))
...
(defn signup-area [data]
  (h/div {:class "signup"}
    ...
    (h/div
      (h/input
        {:id "signup-input"
         :onKeyUp #(reset-in! data [:player-name] (.-value (.-target %)))}))
...
(defn turn-area [data]
  (h/div {:class "turn"}
    (h/span {:class "turn-label"} (localize data :turn))
    (h/span {:class "turn-value"} (:turn @data))))

(defn language-flag [data lang]
  (h/img
   {:src (str "img/flags/" (name lang) ".png")
    :title (localize data lang)
    :class "language-flag"
    :onClick #(reset-in! data [:lang] lang)}))
